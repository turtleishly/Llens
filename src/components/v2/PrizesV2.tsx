...
const prizes = [{
  ...
  benefits: ["Sunway Bursary", "Visit to Google's Office", "6 months Pixlr Premium", "Cert of Commendation"]
}, {
  icon: Medal,
  position: "2ND PLACE",
  amount: "RM 12,000",
...
const PrizesV2 = () => {
  return <section id="prizes" className="py-32 bg-black relative overflow-hidden">
            ...
            fontFamily: "'Orbitron', sans-serif"
          }}>WIN PRIZES<span className="text-cyan-500">PRIZES</span>
                        </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {prizes.map((prize, index) => <BlurFade key={prize.position} delay={0.1 + index * 0.1}>
                            <motion.div className="group relative p-8 bg-zinc-950/80 border border-zinc-800 hover:border-cyan-500/50 transition-all duration-500 flex flex-col h-full overflow-hidden" whileHover={{
            scale: 1.02
          }}>
                                ...
                                <div className="flex items-center gap-4 mb-8">
                                    <div className={`w-14 h-14 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center flex-shrink-0 group-hover:border-cyan-500/30 transition-colors`}>
                                        <prize.icon className={`w-8 h-8 ${prize.color}`} style={{
                  filter: `drop-shadow(0 0 10px ${prize.glow})`
                }} />
                                    </div>
                                    <div>
                                ...
                                <ul className="space-y-4 pt-6 border-t border-zinc-900 group-hover:border-cyan-500/20 transition-colors flex-grow">
                                    {prize.benefits.map(benefit => <li key={benefit} className="flex items-start gap-3 text-xs font-medium text-zinc-400 group-hover:text-zinc-300 transition-colors">
